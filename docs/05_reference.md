...
- **Hangboard** (F51 — a grip is an exercise, a hang is a duration `LoggedSet`): PRs flow through the standard per-exercise path, keyed by `exerciseName` = grip. A **weight** PR for heaviest added load (in `additionalWeightKg`, since a hang is bodyweight-loaded) and a **duration** PR for the longest hang (any duration exercise records a longest-hold PR)
